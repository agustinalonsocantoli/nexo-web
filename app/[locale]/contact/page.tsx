import PageHero from "@/components/PageHero";
import ContactForm from "@/components/contact/ContactForm";
import { getTranslations } from "next-intl/server";

export default async function ContactPage() {
  const t = await getTranslations('contact');

  return (
    <main className="bg-[#fbfbfb]">
      <PageHero title={t('heroTitle')} titlePart2={t('heroTitlePart2')} imageSrc="/bg-contacto-des.jpg" />

      {/* Main content — mobile: columna | desktop: dos columnas (form izq, info der) */}
      <div className="mx-auto max-w-7xl flex flex-col gap-6 px-6 py-8 lg:flex-row lg:items-start lg:gap-12 lg:px-[72px] lg:py-12">

        {/* Columna derecha en desktop (intro + info card) — en mobile aparece primero */}
        <div className="flex flex-col gap-4 lg:order-last lg:flex-1">
          {/* Intro */}
          <div className="flex flex-col gap-2">
            <p className="font-body text-[20px] font-semibold leading-5 text-nexo-dark lg:hidden">
              {t('needHelpMobile')}
            </p>
            <p className="font-body text-base leading-5 text-nexo-dark">
              <span className="lg:hidden">
                {t('introMobile')}
              </span>
              <span className="hidden lg:inline">
                {t('introDesktop')}
              </span>
            </p>
          </div>

          {/* Info card */}
          <div className="flex flex-col gap-3 rounded-[16px] bg-[#262626] p-6">
            {/* Título */}
            <p className="font-heading text-[24px] font-bold uppercase tracking-[0.72px] text-nexo-orange">
              {t('infoTitle')}
            </p>

            {/* Email */}
            <a
              href="mailto:info@nexocrossfit.es"
              className="font-body text-base leading-5 text-[#fbfbfb] underline"
            >
              info@nexocrossfit.es
            </a>

            {/* WhatsApp */}
            <div className="flex items-center gap-1.5">
              {/* Icono WhatsApp */}
              <svg
                className="h-5 w-5 shrink-0"
                viewBox="0 0 24 24"
                fill="#25D366"
                xmlns="http://www.w3.org/2000/svg"
                aria-hidden="true"
              >
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
              </svg>
              <a href="https://wa.me/34661388984" target="_blank" rel="noopener noreferrer" className="font-body text-base leading-5 text-[#fbfbfb] transition-opacity hover:opacity-75">
                <span className="font-semibold">WhatsApp:</span> <span className="underline underline-offset-2">661 388 984</span>
              </a>
            </div>

            {/* Dirección */}
            <a href="https://www.google.com/maps/place//data=!4m2!3m1!1s0xd604f5651fcf9db:0x5f81e13cf272fac9?sa=X&ved=1t:8290&hl=es-419&ictx=111&cshid=1772821693809793" target="_blank" rel="noopener noreferrer" className="font-body text-base leading-5 text-[#fbfbfb] underline underline-offset-2 transition-opacity hover:opacity-75">
              {t('addressLabel')}<span className="font-semibold">{t('addressValue')}</span>
            </a>
          </div>
        </div>

        {/* Formulario (client component) */}
        <ContactForm />
      </div>
    </main>
  );
}
